      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <div className="footer-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '3rem', marginBottom: '3rem' }}>
          ...
          <div>
            ...
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.82rem', fontWeight: 300, color: 'rgba(253,248,240,0.45)', lineHeight: 1.8 }}>
              {c.address ? c.address.split(',')[0] : '12 Victoria Island Boulevard'}<br />
              {c.address && c.address.includes(',') ? c.address.substring(c.address.indexOf(',') + 1).trim() : 'Lagos, Nigeria'}<br /><br />
              {c.phone || '+234 801 234 5678'}<br />
              {c.email || 'hello@adamsrestaurant.com'}
            </p>
          </div>
          ...
          <div>
            ...
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.82rem', fontWeight: 300, color: 'rgba(253,248,240,0.45)', lineHeight: 2 }}>
              {c.hours ? c.hours.split('|').map((h, i) => <span key={i}>{h.trim()}<br/></span>) : (
                <>Monday – Friday<br />8:00 AM – 10:00 PM<br /><br />Saturday – Sunday<br />9:00 AM – 11:00 PM</>
              )}
            </p>
          </div>
        </div>

        <div className="footer-bottom" style={{ borderTop: '1px solid rgba(201,168,76,0.08)', paddingTop: '2rem', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', textAlign: 'center' }}>
